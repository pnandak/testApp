...
#					} else {
#						myFile <- RCurl::getURL(url=paste0(ghubURL, input$choiceCorpus, ".txt"), ssl.verifypeer=FALSE)
...
output$lowerFreqSlider<- renderUI({
					...
					min = round(lowerFreqRange()[1]), max = round(lowerFreqRange()[2]), value = round(lowerFreqRange()[1]), 
					step = NULL, ticks = TRUE)
			})


finalUnigramMatrix <- reactive({
				if(input$selectFeaturesBtn == 0)
				return()
				isolate({
